...
func (w *WebModuleBase) configJSHandler(rw http.ResponseWriter, r *http.Request) {
	...
	if runtimeEnv != nil {
		...
		for key, value := range runtimeEnv {
			...
			first = false
			// Escape the key and value for safe JavaScript embedding
			escapedKey := escapeJSString(key)
			escapedValue := escapeJSString(value)
			sb.WriteString(fmt.Sprintf("  %q: %q", escapedKey, escapedValue))
		}
		if !first {
			sb.WriteString("\n")
		}
...
// escapeJSString escapes a string for safe inclusion in JavaScript
// This prevents XSS attacks from malicious configuration values
