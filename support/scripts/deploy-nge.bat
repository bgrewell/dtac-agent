...
(for %%a in (%systems%) do (
    ...
    ssh intel@%%a -C 'sudo mv ~/system-agentd /opt/system-agent/bin/.'
    ssh intel@%%a -C 'sudo mv ~/system-agentd.service /lib/systemd/system/.'
    ssh intel@%%a -C 'sudo systemctl daemon-reload'
    ssh intel@%%a -C 'sudo systemctl start system-agentd'
